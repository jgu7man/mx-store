...
export class MxStorePublicService {
    constructor(
    ...
    async getArticulosFilter( categoria: string, limit?: number,  order?: string ) {
        ...
        var tiendaRef = this.fs.collection<MxStoreProductModel>('tienda/productos/referencias').ref
        var docs = !limit ?
    ...
    async getMoreProducts( categoria: string, limit?: number, after?: string ,order?: string ) {
        ...
        var docs = await tiendaRef.where( 'categorias', 'array-contains', categoria )
            .limit( limit || 0 ).orderBy( order, 'asc' ).startAfter(after).get()
    ...
    setSliderContent(content: string) {
        this.$sliderContent.next(content)
        return
    }

    async getProduct( idProduct: string ) {
        var tiendaRef = this.fs.collection<any>( 'tienda/productos/referencias' ).ref
